<?php
	...
	class Database {
		...
		function countSel($table, $columns, $where, $array) {
			$db = $this->connect();

			$preparedStatement = $db->prepare("SELECT COUNT($columns) AS jobNum FROM $table WHERE $where");
		...
		function search($table, $columns, $where, $sort, $page, $limit) {
			...
			if ( $this->_limit == 0 ) {
				$preparedStatement = $db->prepare("SELECT $columns FROM $table WHERE $where ORDER BY ".$this->_sort." ".$this->_run."");
			} else {

				$preparedStatement = $db->prepare("SELECT $columns FROM $table WHERE $where ORDER BY ".$this->_sort." ".$this->_run." LIMIT  	" . ( ( $this->_page - 1 ) * $this->_limit ) . ", $this->_limit");
			}
